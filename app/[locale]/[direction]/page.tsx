  ...
  return (
    <>
      ...
      */}
      ...
      {/*
        BUSINESS собирается отдельным шаблоном. Состав и порядок разделов те
        же, что ниже, — своя у ветки только вёрстка: тональная лестница,
        двенадцать колонок с асимметрией и кадры разных пропорций. Выразить это
        флагами в общей разметке значило бы протащить арт-дирекцию одной ветки
        через шаблоны всех трёх; здесь же PRIVATE и PRODUCTION к ней даже не
        прикасаются.
      */}
      {direction === 'business' ? (
        <BusinessHome
