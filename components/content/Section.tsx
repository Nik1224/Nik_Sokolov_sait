...
type Props = {
  ...
  className?: string;
  /**
   * Подложка секции. `raised` заливает полосу во всю ширину экрана — контент
   * при этом остаётся в контейнере.
   *
   * Страница ветки — семь тысяч пикселей на одном и том же чёрном, и глазу не
   * за что зацепиться: разделы отличаются только тем, что написано в
   * заголовке. Полосы делают из ленты главы — видно, где кончается одно и
   * начинается другое, ещё до того, как прочитан заголовок. Через одну, а не
   * подряд: чередование и есть весь смысл, две полосы встык снова сливаются.
   */
  surface?: 'ink' | 'raised';
...
export function Section({
  ...
  return (
    <section id={id} className={surface === 'raised' ? 'bg-ink-raised' : undefined}>
      <div className={`container-content py-[var(--spacing-section)] ${className}`}>
 ...
 * Пустой раздел.
 *
 * По возможности с выходом: «здесь пока пусто» без продолжения — тупик, и
 * человек уходит с сайта, а не в соседний раздел. Ссылки нет только тогда,
 * когда идти действительно некуда.
 */
  ...
  return (
    <div className="border border-dashed border-line px-6 py-16 text-center">
      ...
      {action ? (
        <Link
          href={action.href}
          className="label mt-8 inline-block border border-line-strong px-7 py-4 text-bone transition-colors hover:border-bone"
        >
          {action.label} →
        </Link>
      ) : null}
