...
export const direction = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'clients',
      title: 'С кем работали',
      description:
        'Названия компаний для полосы доверия на главной. Логотипы не нужны — имя набирается текстом. Ставится только с подтверждения заказчика (§1.2). Меньше трёх названий лучше не показывать вовсе.',
      type: 'array',
      of: [
        {
          type: 'object',
          fields: [
            ...
            { name: 'note', title: 'Что снимали', type: 'localeString' },
            {
              name: 'logo',
              title: 'Логотип',
              description:
                'Только официальный файл от клиента, лучше SVG. Перерисовывать, перекрашивать и менять пропорции чужой знак нельзя. Пусто — имя набирается текстом, и это рабочий вид.',
              type: 'image',
            },
          ],
          preview: { select: { title: 'name', subtitle: 'note.ru', media: 'logo' } },
        },
      ],
    }),
    defineField({
