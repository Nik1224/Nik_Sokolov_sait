    ...
    pricingGroups: [
      {
        ...
        description: {
          ru: 'Фото, видео или оба формата сразу. За оба формата скидка 10% уже в цене.',
          en: 'Photo, video or both at once. Taking both takes 10% off the price.',
        },
      },
      {
        slug: 'portrait',
...
/**
 * Петля для наведения на плитку категории. Вертикальная, без звука, ~640 px
 * по ширине: на плитке видно горизонтальную полосу из середины кадра.
 *
 * Подключается только при первом наведении (`preload="none"`), поэтому пять
 * роликов не весят ничего до тех пор, пока человек не поведёт мышью.
 */
function preview(slug: string, ru: string, en: string, height: number): MediaAsset {
  return {
    _key: `cat-preview-${slug}`,
    type: 'video',
    provider: 'file',
    poster: { src: `/media/category-${slug}-poster.jpg`, width: 720, height },
    loopSrc: `/media/category-${slug}-loop.mp4`,
    alt: { ru, en },
    rights: 'owned',
  };
}
...
export const testimonials: Testimonial[] = [
  ...
  {
    /*
     * Отзыв из переписки после съёмки обучающих роликов. Имя компании и сам
     * отзыв владелец согласовал — без этого ни то, ни другое на сайт не идёт
     * (§5.2, §8). Эмодзи из сообщения сняты: они читаются как оформление,
     * а не как слова, а слова здесь и есть отзыв.
     */
    _id: 'tst.ekaterina-epica',
    author: 'Екатерина, EPICA Professional',
    directions: ['business'],
    order: 2,
    source: 'Переписка после съёмки',
    text: {
      ru: 'Отличные фото, ты настоящий волшебник!',
    },
  },
  {
    _id: 'tst.sergey-band',
