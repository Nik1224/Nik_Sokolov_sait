...

  {
    ...
    body: bodyRuEn(
      [
        ...
        'Четвёртое — свет, и это то, о чём я говорю каждому клиенту. Рестораны и банкетные залы почти всегда тёмные, а верхний свет там холодный и плоский: он выравнивает лица, убивает объём и красит кожу в серый. Никакая камера этого не исправляет — она может только вытянуть кадр из темноты, но не сделать его красивым.',
        'Поэтому мой совет однозначный: ставьте профессиональный свет и берите человека, который им управляет. Не приборы «на потом разберёмся», а световика, который выставит схему под зал, а потом будет вести её весь вечер — застолье, танцы и финал требуют разного, и переставлять придётся не раз.',
        'Отдельно уточню, потому что путают постоянно. Речь не о том свете, который привозят с собой ведущие и диджеи. Цветные головы и заливка зала синим сделаны для танцпола и работают на настроение толпы, а не на лица. На фотографиях они дают цветные пятна на коже, которые не убираются в обработке. Профессиональный свет — это про то, чтобы люди в кадре выглядели людьми.',
        'И про выбор, который возникает почти всегда, когда бюджет не резиновый. Если решать между светом и, скажем, живыми цветами вместо искусственных — берите свет. Цветы украшают один угол, и на фотографии разницу между живыми и искусственными чаще всего никто не заметит. Свет преображает всю площадку целиком: тот же зал при поставленном свете выглядит дороже, чем при верхнем освещении с любым декором.',
      ...
      [
        ...
        'Fourth: light, and this is what I say to every client. Restaurants and banqueting halls are almost always dark, and their overhead light is cold and flat — it flattens faces, kills depth and turns skin grey. No camera fixes that. A camera can pull a frame out of darkness; it cannot make it beautiful.',
        'So my advice is unambiguous: bring in professional lighting and bring the person who runs it. Not fixtures to be figured out later, but a lighting technician who builds a setup for the room and then keeps it running all evening — dinner, dancing and the finale each need something different, and it has to be adjusted more than once.',
        'One clarification, because this is constantly confused. This is not the lighting that hosts and DJs bring with them. Moving heads and washing the room in blue are made for a dance floor and work on the mood of a crowd, not on faces. In photographs they leave coloured patches on skin that cannot be edited out. Professional lighting is about people in frame looking like people.',
        'And about the choice that comes up whenever the budget is finite. If it is between lighting and, say, real flowers instead of artificial ones — take the lighting. Flowers decorate one corner, and in a photograph almost nobody can tell real from artificial. Light transforms the entire venue: the same room, properly lit, looks more expensive than it does under ceiling lights with any decor at all.',
