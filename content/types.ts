...
export type Category = {
  ...
  preview?: MediaAsset;
  /**
   * В какой ряд фильтров попадает категория.
   *
   * Названия категорий сидят на двух разных осях. «Реклама» отвечает на вопрос
   * «что за ролик», «Спорт» — на вопрос «что снимаем». В одном ряду это
   * несравнимые пункты, и человек не понимает, по какому признаку выбирает.
   *
   * Поэтому ряд разводится надвое. Пусто — категория идёт в общий ряд без
   * заголовка, как было раньше.
   */
  filterGroup?: 'format' | 'subject';
  /**
...
export type DirectionDoc = {
  ...
  /**
   * Компании, которые уже заказывали. У бизнес-ветки это второй вопрос после
   * «что снимаете»: маркетолог смотрит не на перечень услуг, а на то, кто до
   * него доверил задачу.
   *
   * Логотипов здесь нет намеренно. Чужой знак требует разрешения на
   * использование и своей сетки размеров, а набранное имя не требует ни того,
   * ни другого — и в монохромной вёрстке читается ровнее пёстрого ряда.
   *
   * Имя ставится только с подтверждения заказчика (§1.2, §5.5). Пусто — блока
   * нет: полоса из двух названий работает против доверия, а не на него.
   */
  clients?: {
    name: string;
    /**
     * Имя для бегущей строки, когда полное в неё не помещается.
     *
     * Полоса держится на том, что названия сопоставимы по весу: «Федерация
     * спортивной гимнастики и акробатики» рядом с «Panthenol» — это сорок три
     * знака против девяти, и один заказчик занимает столько же места, сколько
     * четверо остальных вместе. Сокращать в компоненте нельзя: обрезка по
     * числу букв рубит слова посередине и портит чужое имя. Поэтому короткая
     * форма задаётся руками и ровно там, где лежит полная.
     *
     * Пусто — идёт `name`, и это обычный случай: сокращение нужно двоим из
     * десяти.
     */
    short?: string;
    note?: LocaleString;
    /**
     * Логотип. Только официальный файл от самого клиента: чужой знак нельзя
     * ни перерисовывать, ни перекрашивать, ни менять его пропорции. Пока файла
     * нет, имя набирается текстом — это не заглушка, а рабочий вид: набранное
     * имя честнее пересобранного логотипа.
     *
     * `height` — оптическая высота в пикселях при базовом кегле. У знаков
     * разной формы одинаковая рамка даёт разный визуальный вес, и ряд
     * разваливается; число подбирается на глаз для каждого.
     */
